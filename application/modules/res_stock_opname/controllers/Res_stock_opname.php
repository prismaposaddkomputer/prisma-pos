...
class Res_stock_opname extends MY_Restaurant {
  ...
  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'res_stock_opname'){
      $this->session->set_userdata(array('menu' => 'res_stock_opname'));
      $this->session->unset_userdata('search_term');
    }
    $this->load->model('app_config/m_res_config');

    $this->role_id = $this->session->userdata('role_id');
    $this->module_controller = 'res_stock_opname';
    $this->access = $this->m_res_config->get_permission($this->role_id, $this->module_controller);

    $this->load->model('m_res_stock_opname');
  }
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $config['base_url'] = base_url().'res_stock_opname/index/';
      $config['per_page'] = 10;

      $from = $this->uri->segment(3);

      if($this->session->userdata('search_term') == null){
        $num_rows = $this->m_res_stock_opname->num_rows();

        $config['total_rows'] = $num_rows;
        $this->pagination->initialize($config);

        $data['stock_opname'] = $this->m_res_stock_opname->get_list($config['per_page'],$from,$search_term = null);
      }else{
        $search_term = $this->session->userdata('search_term');
        $num_rows = $this->m_res_stock_opname->num_rows($search_term);
        $config['total_rows'] = $num_rows;
        $this->pagination->initialize($config);

        $data['stock_opname'] = $this->m_res_stock_opname->get_list($config['per_page'],$from,$search_term);
      }

      $this->view('res_stock_opname/index',$data);
    } else {
  ...
  {
    $this->session->unset_userdata('search_term');
    redirect(base_url().'res_stock_opname/index');
  }
  ...
  public function form($id = null)
  {
    ...
    if ($id == null) {
      if ($this->access->_create == 1) {
        $last = $this->m_res_stock_opname->get_last();
        if ($last == null) {
          $data['tx_id'] = 1;
        ...
        $data['item'] = $this->m_res_stock_opname->get_stock_last();
        $this->view('res_stock_opname/form', $data);
      } else {
        redirect(base_url().'app_error/error/403');
      }
    }else{
      if ($this->access->_update == 1) {
        $data['last'] = $this->m_res_stock_opname->get_last();
        if($data['last'] == null){
        ...
        $data['stock_opname'] = $this->m_res_stock_opname->get_by_id($id);
        $this->view('res_stock_opname/form', $data);
      } else {
        redirect(base_url().'app_error/error/403');
      }
  ...
  public function insert()
  {
    ...
    $this->m_res_stock_opname->insert($data_opname);
    ...
    foreach ($data['item_id'] as $key => $val) {
      ...
      $this->m_res_stock_opname->insert_detail($data_item);
    }
  ...
  public function update()
  {
    ...
    $this->m_res_stock_opname->update($tx_id, $data_opname);
    $this->load->model('res_stock/m_res_stock');
    $this->load->model('m_res_stock_opname');
    foreach ($data['item_id'] as $key => $val) {
      $data_item = null;
      ...
      $data_item['stock_now'] = $data['stock_now'][$key];
      $data_item['stock_last'] = $data['stock_last'][$key];
      $this->m_res_stock_opname->update_detail($stock_id, $data_item);
    }
  ...
  public function detail($tx_id,$tx_type)
  {
    ...
    $data['detail'] = $this->m_res_stock_opname->get_detail($tx_id,$tx_type);

    $this->view('res_stock_opname/detail',$data);
  }
