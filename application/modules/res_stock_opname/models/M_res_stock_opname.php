...
class M_res_stock_opname extends CI_Model {

	public function get_list($number,$offset,$search_term = null)
  {
		if($search_term == null){
			return $this->db
				->join('res_tx_type','res_stock_opname.tx_type = res_tx_type.tx_type')
				->order_by('tx_date','desc')
				->order_by('tx_id','desc')
				->get('res_stock_opname',$number,$offset)
				->result();
		}else{
  ...
  public function get_by_id($id)
  {
	...
	public function get_opname_detail($tx_id)
	{
      ...
      FROM res_stock_opname_detail a
			LEFT JOIN(
				SELECT
					item_id,
			...
			// ->join('res_item','res_stock_opname_detail.item_id = res_item.item_id')
			// ->where('tx_id', $tx_id)
			// ->get('res_stock_opname_detail')->result();
	}
      ...
      LEFT JOIN (
			...
			LEFT JOIN(
				...
				WHERE
					...
					tx_type = 'TXI'
				GROUP BY item_id
			) t ON (res_item.item_id = t.item_id)"
    );
  ...
  {
    $this->db->insert('res_stock_opname',$data);
  }

	public function insert_detail($data)
  {
    $this->db->insert('res_stock_opname_detail',$data);
  }

  public function update($id,$data)
  {
    $this->db->where('tx_id',$id)->update('res_stock_opname',$data);
  }

	public function update_detail($id,$data)
  {
    $this->db->where('stock_id',$id)->update('res_stock_opname_detail',$data);
  }

  public function delete($id)
  {
    $this->db->where('stock_opname_id',$id)->update('res_stock_opname',array('is_deleted' => '1'));
  }

	function num_rows($search_term = null){
		if($search_term == null){
			return $this->db->get('res_stock_opname')->num_rows();
		}else{
			$tanggal = explode(" - ", $search_term);
			$date_start = ind_to_date($tanggal[0]);
			$date_end = ind_to_date($tanggal[1]);
