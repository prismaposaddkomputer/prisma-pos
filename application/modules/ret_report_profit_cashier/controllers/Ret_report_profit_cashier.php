...
class Ret_report_profit_cashier extends MY_Retail {

  var $access, $report_profit_cashier_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'ret_report_profit_cashier'){
      $this->session->set_userdata(array('menu' => 'ret_report_profit_cashier'));
    ...
    $this->module_controller = 'ret_report_profit_cashier';
    $this->access = $this->m_ret_config->get_permission($this->role_id, $this->module_controller);

    $this->load->model('m_ret_report_profit_cashier');
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $this->view('ret_report_profit_cashier/index',$data);
    } else {
  ...
  public function action()
  {
    ...
    switch ($data['type']) {
      ...
      case 'annual':
        $year = $data['year'];
        redirect(base_url().'ret_report_profit_cashier/annual/'.$year.'/'.$user_id);
        break;

      case 'monthly':
        $month = ind_to_month($data['month']);
        redirect(base_url().'ret_report_profit_cashier/monthly/'.$month.'/'.$user_id);
        break;

      case 'weekly':
        $week = $data['week'];
        $week = str_replace(' - ', '/', $week);
        redirect(base_url().'ret_report_profit_cashier/weekly/'.$week.'/'.$user_id);
        break;

      case 'daily':
        $date = ind_to_date($data['date']);
        redirect(base_url().'ret_report_profit_cashier/daily/'.$date.'/'.$user_id);
        break;

      case 'range':
        $range = $data['range'];
        $range = str_replace(' - ', '/', $range);
        redirect(base_url().'ret_report_profit_cashier/range/'.$range.'/'.$user_id);
        break;
  ...
  public function annual($year,$user_id)
  {
    ...
    $data['annual'] = $this->m_ret_report_profit_cashier->annual($year,$user_id);
  ...
  public function monthly($month,$user_id)
  {
    ...
    $data['monthly'] = $this->m_ret_report_profit_cashier->monthly($month,$user_id);
  ...
  public function weekly($date_start, $date_end, $user_id)
  {
    ...
    $data['weekly'] = $this->m_ret_report_profit_cashier->weekly(ind_to_date($date_start),ind_to_date($date_end),$user_id);
  ...
  public function daily($date, $user_id)
  {
    ...
    $data['daily'] = $this->m_ret_report_profit_cashier->daily($date,$user_id);
  ...
  public function range($date_start, $date_end, $user_id)
  {
    ...
    $data['range'] = $this->m_ret_report_profit_cashier->range(ind_to_date($date_start),ind_to_date($date_end),$user_id);
  ...
  public function detail($tx_id)
  {
    ...
    $data['billing'] = $this->m_ret_report_profit_cashier->detail($tx_id);
