...
class Kar_payment extends MY_Karaoke {

  var $access, $payment_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'kar_payment'){
      $this->session->set_userdata(array('menu' => 'kar_payment'));
      $this->session->unset_userdata('search_term');
    }
    $this->load->model('app_config/m_kar_config');

    $this->role_id = $this->session->userdata('role_id');
    $this->module_controller = 'kar_payment';
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $this->view('kar_payment/index',$data);
    } else {
  ...
  {
    $this->session->unset_userdata('search_term');
    redirect(base_url().'kar_payment/index');
  }
  ...
  public function form($id = null)
  {
    ...
    if ($id == null) {
      if ($this->access->_create == 1) {
        ...
        $data['action'] = 'insert';
        $data['payment'] = null;
        $this->view('kar_payment/form', $data);
      } else {
        redirect(base_url().'app_error/error/403');
      }
    }else{
      if ($this->access->_update == 1) {
        ...
        $data['payment'] = $this->m_kar_payment->get_payment();
  ...
  public function insert()
  {
    ...
    $this->m_kar_payment->insert($data);
  ...
  public function edit($id)
  {
    ...
    $this->load->view('kar_payment/update', $data);
  }
  ...
  public function update()
  {
    ...
    $pajak = $this->m_kar_booking->get_pajak();
    ...
    $this->m_kar_payment->update($id,$data);
    ...
    $bill = $this->m_kar_payment->get_by_billing($id);
  ...
  public function struk($id)
  {
    ...
    $client = $this->m_kar_payment->get_client();
    $guest = $this->m_kar_booking->get_all_tamu();
    $room = $this->m_kar_booking->get_all_room();
    $diskon = $this->m_kar_booking->get_all_diskon();
    $service = $this->m_kar_booking->get_all_service();
    $tipe = $this->m_kar_booking->get_tipe();
    $pajak = $this->m_kar_booking->get_pajak();
    $payment = $this->m_kar_payment->get_payment();
    $booking = $this->m_kar_booking->get_by_id($id);
    $client = $this->m_kar_client->get_all();
    ...
    try {
      ...
      foreach($payment as $t){
        if($booking->booking_id==$t->booking_id){
            ...
            $printer -> feed();
            $printer -> text('Pajak karaoke ('.$pajak['tax_ratio'].'%) = '.num_to_price($p));
            $printer -> feed();
