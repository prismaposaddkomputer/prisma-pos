...
class M_par_config extends CI_Model {

  public function get_permission($role_id, $module_controller)
  {
    $query = $this->db
      ->select('module_icon,_create, _read, _update, _delete')
      ->where('par_module.module_controller', $module_controller)
      ->where('par_permission.role_id', $role_id)
      ->join('par_module','par_permission.module_id = par_module.module_id')
      ->get('par_permission');
    return $query->row();
  }
