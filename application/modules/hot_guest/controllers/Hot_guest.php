...
class Hot_guest extends MY_Hotel {

  var $access, $guest_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'hot_guest'){
      $this->session->set_userdata(array('menu' => 'hot_guest'));
      $this->session->unset_userdata('search_term');
    }
    $this->load->model('app_config/m_hot_config');

    $this->role_id = $this->session->userdata('role_id');
    $this->module_controller = 'hot_guest';
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $config['base_url'] = base_url().'hot_guest/index/';
      $config['per_page'] = 10;

      $from = $this->uri->segment(3);

      if($this->session->userdata('search_term') == null){
        $num_rows = $this->m_hot_guest->num_rows();
  ...
  {
    $this->session->unset_userdata('search_term');
    redirect(base_url().'hot_guest/index');
  }
  ...
  public function update()
  {
    ...
    $this->m_hot_guest->update($id,$data);
