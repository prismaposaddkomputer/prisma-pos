  ...
  <div class="row">
    <div class="col-md-12">
      <form method="post" action="<?=base_url()?>res_report_selling_user/frame_pdf">
        <input type="hidden" name="url" value="<?=base_url()?>res_report_selling_user/weekly_pdf/<?=$date_start?>/<?=$date_end?>/<?=$user_id?>">
        <button class="btn btn-primary" type="submit"><i class="fa fa-file-pdf-o"></i> Download PDF</button>
        <a href="<?=base_url()?>res_report_selling_user/weekly_print/<?=$date_start?>/<?=$date_end?>/<?=$user_id?>" class="btn btn-warning"><i class="fa fa-print"></i> Print Laporan</a>
      </form>
      <br>
