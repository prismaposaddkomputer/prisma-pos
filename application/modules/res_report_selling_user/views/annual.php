<div class="content-header">
  <h4>
    <a href="<?=base_url('res_report_selling_user')?>" class="btn btn-success"><i class="fa fa-arrow-left"></i></a> 
  ...
  <div class="row">
    <div class="col-md-12">
      <form method="post" action="<?=base_url()?>res_report_selling_user/frame_pdf">
        <input type="hidden" name="url" value="<?=base_url()?>res_report_selling_user/annual_pdf/<?=$year?>/<?=$user_id?>">
        <button class="btn btn-primary" type="submit"><i class="fa fa-file-pdf-o"></i> Download PDF</button>
        <a href="<?=base_url()?>res_report_selling_user/annual_print/<?=$year?>/<?=$user_id?>" class="btn btn-warning"><i class="fa fa-print"></i> Print Laporan</a>
      </form>
      <br>
            ...
            <?php $i=1;foreach ($annual as $row): ?>
              <tr>
                ...
                <td class="text-center">
                  <a href="<?=base_url()?>res_report_selling_user/monthly/<?=$row->tx_year?>-<?=str_pad($row->tx_month, 2, '0', STR_PAD_LEFT)?>/<?=$user_id?>" class="btn btn-xs btn-success"><i class="fa fa-list"></i> </a>
                </td>
