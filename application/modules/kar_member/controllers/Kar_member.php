...
class Kar_member extends MY_Karaoke {

  var $access, $member_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'kar_member'){
      $this->session->set_userdata(array('menu' => 'kar_member'));
      $this->session->unset_userdata('search_term');
    }
    $this->load->model('app_config/m_kar_config');

    $this->role_id = $this->session->userdata('role_id');
    $this->module_controller = 'kar_member';
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $config['base_url'] = base_url().'kar_member/index/';
      $config['per_page'] = 10;

      $from = $this->uri->segment(3);

      if($this->session->userdata('search_term') == null){
        $num_rows = $this->m_kar_member->num_rows();
  ...
  {
    $this->session->unset_userdata('search_term');
    redirect(base_url().'kar_member/index');
  }
  ...
  public function update()
  {
    ...
    $this->m_kar_member->update($id,$data);
