...
class Hot_report_booking extends MY_Hotel {

  var $access, $brand_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'hot_report_booking'){
      $this->session->set_userdata(array('menu' => 'hot_report_booking'));
      $this->session->unset_userdata('search_term');
    }
    $this->load->model('app_config/m_hot_config');

    $this->role_id = $this->session->userdata('role_id');
    $this->module_controller = 'hot_report_booking';
    $this->access = $this->m_hot_config->get_permission($this->role_id, $this->module_controller);

    $this->load->model('m_hot_report_booking');
  }
	...
	public function index($type = null)
  {
    if ($this->access->_read == 1) {
      ...
      $this->view('hot_report_booking/index',$data);
    } else {
  ...
  public function report_action($value='')
  {
    ...
    switch ($data['type']) {
      ...
      case 'annual':
        $year = $data['year'];
        redirect(base_url().'hot_report_booking/report_annual/'.$year);
        break;

      case 'monthly':
        $month = ind_to_month($data['month']);
        redirect(base_url().'hot_report_booking/report_monthly/'.$month);
        break;

      case 'weekly':
        if($data['week'] == ''){
          redirect(base_url().'hot_report_booking/index/weekly');
        };
        $week = $data['week'];
        $week = str_replace(' - ', '/', $week);
        redirect(base_url().'hot_report_booking/report_weekly/'.$week);
        break;

      case 'daily':
        $date = ind_to_date($data['date']);
        redirect(base_url().'hot_report_booking/report_daily/'.$date);
        break;

      case 'range':
        $range = $data['range'];
        $range = str_replace(' - ', '/', $range);
        redirect(base_url().'hot_report_booking/report_range/'.$range);
        break;
    }
  }
  ...
  public function report_monthly($month)
  {
    ...
    $data['guest'] = $this->m_hot_report_booking->get_all_tamu();
    $data['room'] = $this->m_hot_report_booking->get_all_room();
    $data['service'] = $this->m_hot_report_booking->get_all_service();
    $data['tipe'] = $this->m_hot_report_booking->get_tipe();
    $data['month'] = $month;
  ...
  public function report_daily($date)
  {
    ...
    $data['booking'] = $this->m_hot_report_booking->report_daily($date);
    $data['date'] = $date;
    $data['guest'] = $this->m_hot_report_booking->get_all_tamu();
    $data['room'] = $this->m_hot_report_booking->get_all_room();
    $data['service'] = $this->m_hot_report_booking->get_all_service();
    ...
    $this->view('hot_report_booking/report_daily', $data);
  }
  ...
  public function report_range($date_start, $date_end)
  {
    ...
    $data['booking'] = $this->m_hot_report_booking->report_range(ind_to_date($date_start),ind_to_date($date_end));    
    $data['guest'] = $this->m_hot_report_booking->get_all_tamu();
    $data['room'] = $this->m_hot_report_booking->get_all_room();
    $data['service'] = $this->m_hot_report_booking->get_all_service();
    $data['tipe'] = $this->m_hot_report_booking->get_tipe();
