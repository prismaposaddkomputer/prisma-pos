...
class Kar_module extends MY_Karaoke {
	...
	public function index()
  {
    if($this->access->_read == 1){
      ...
      $config['base_url'] = base_url().'kar_module/index/';

      if($this->session->userdata('search_term') == null){
        $nukar_rows = $this->m_kar_module->num_rows();
        $data['module'] = $this->m_kar_module->get_list();
      }else{
        $search_term = $this->session->userdata('search_term');
        $data['module'] = $this->m_kar_module->get_list($search_term);
      }
  ...
  {
    $this->session->unset_userdata('search_term');
    redirect(base_url().'kar_module/index');
  }

  public function form($id = null)
  {
    $data['module_all'] = $this->m_kar_module->get_all();
    $data['access'] = $this->access;
    if ($id == null) {
      ...
      if ($this->access->_update == 1) {
        ...
        $data['module'] = $this->m_kar_module->get_by_id($id);
        $data['action'] = 'update';
        $this->view('form', $data);
      }else{
        redirect(base_url().'app_error/error/403');
  ...
  {
    $data['module']= $this->m_kar_module->get_specific($id);
  ...
  public function update()
  {
    ...
    unset($data['module_id_old']);
    $this->m_kar_module->update($id,$data);
