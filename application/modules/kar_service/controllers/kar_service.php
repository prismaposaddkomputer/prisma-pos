...
class Kar_service extends MY_Karaoke {

  var $access, $service_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'kar_service'){
      $this->session->set_userdata(array('menu' => 'kar_service'));
      $this->session->unset_userdata('search_term');
    }
    $this->load->model('app_config/m_kar_config');

    $this->role_id = $this->session->userdata('role_id');
    $this->module_controller = 'kar_service';
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $config['base_url'] = base_url().'kar_service/index/';
      $config['per_page'] = 10;

      $from = $this->uri->segment(3);

      if($this->session->userdata('search_term') == null){
        $num_rows = $this->m_kar_service->num_rows();

        $config['total_rows'] = $num_rows;
        $this->pagination->initialize($config);

        $data['service'] = $this->m_kar_service->get_list($config['per_page'],$from,$search_term = null);
      }else{
        $search_term = $this->session->userdata('search_term');
        $num_rows = $this->m_kar_service->num_rows($search_term);
        $config['total_rows'] = $num_rows;
        $this->pagination->initialize($config);

        $data['service'] = $this->m_kar_service->get_list($config['per_page'],$from,$search_term);
      }

      $this->view('kar_service/index',$data);
    } else {
  ...
  {
    $data['service']= $this->m_kar_service->get_specific($id);
    $this->load->view('kar_service/update', $data);
  }
