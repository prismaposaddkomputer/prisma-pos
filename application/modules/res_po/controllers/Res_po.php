...
class Res_po extends MY_Restaurant {

  var $access, $role_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'res_po'){
      $this->session->set_userdata(array('menu' => 'res_po'));
      $this->session->unset_userdata('search_term');
    }
    $this->load->model('app_config/m_res_config');

    $this->role_id = $this->session->userdata('role_id');
    $this->module_controller = 'res_po';
    $this->access = $this->m_res_config->get_permission($this->role_id, $this->module_controller);

    $this->load->model('m_res_po');
  }
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $config['base_url'] = base_url().'res_po/index/';
      $config['per_page'] = 10;

      $from = $this->uri->segment(3);

      if($this->session->userdata('search_term') == null){
        $num_rows = $this->m_res_po->num_rows();

        $config['total_rows'] = $num_rows;
        $this->pagination->initialize($config);

        $data['po'] = $this->m_res_po->get_list($config['per_page'],$from,$search_term = null);
      }else{
        $search_term = $this->session->userdata('search_term');
        $num_rows = $this->m_res_po->num_rows($search_term);
        $config['total_rows'] = $num_rows;
        $this->pagination->initialize($config);

        $data['po'] = $this->m_res_po->get_list($config['per_page'],$from,$search_term);
      }

      $this->view('res_po/index',$data);
    } else {
  ...
  {
    $this->session->unset_userdata('search_term');
    redirect(base_url().'res_po/index');
  }
  ...
  public function form($id = null)
  {
    ...
    $this->load->model('res_supplier/m_res_supplier');
    $data['supplier'] = $this->m_res_supplier->get_all();
    if ($id == null) {
      if ($this->access->_create == 1) {
        $last = $this->m_res_po->get_last();
        if ($last == null) {
          $data['tx_id'] = 1;
      ...
      if ($this->access->_update == 1) {
        ...
        $data['action'] = 'update';
        $data['po'] = $this->m_res_po->get_detail($id,'TXP');
  ...
  public function insert()
  {
    ...
    foreach ($data['item_id'] as $key => $val) {
      ...
      $this->m_res_po->insert_detail($data_item);
    }
  ...
  public function update()
  {
    ...
    if ($data['tx_status'] == -1) {
      ...
      $this->m_res_po->update($tx_id,$data_po);
      //load model
      ...
      foreach ($data['item_id'] as $key => $val) {
        ...
        $this->m_res_po->update_detail($stock_id,$data_item);
        //insert into stock
        if ($data['tx_status'] == 1) {
          $data_stock = array(
            'tx_id' => $data['tx_id'],
  ...
  public function detail($tx_id,$tx_type)
  {
    ...
    $data['detail'] = $this->m_res_po->get_detail($tx_id,$tx_type);

    $this->view('res_po/detail',$data);
  }
