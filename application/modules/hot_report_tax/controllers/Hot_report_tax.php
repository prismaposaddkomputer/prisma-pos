...
class Hot_report_tax extends MY_Hotel {

  var $access, $report_selling_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'hot_report_tax'){
      $this->session->set_userdata(array('menu' => 'hot_report_tax'));
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $this->view('hot_report_tax/index',$data);
    } else {
  ...
  public function action()
  {
    ...
    switch ($data['type']) {
      ...
      case 'annual':
        $year = $data['year'];
        redirect(base_url().'hot_report_tax/annual/'.$year);
        break;

      case 'monthly':
        $month = ind_to_month($data['month']);
        redirect(base_url().'hot_report_tax/monthly/'.$month);
        break;

      case 'weekly':
        $week = $data['week'];
        $week = str_replace(' - ', '/', $week);
        redirect(base_url().'hot_report_tax/weekly/'.$week);
        break;

      case 'daily':
        $date = ind_to_date($data['date']);
        redirect(base_url().'hot_report_tax/daily/'.$date);
        break;

      case 'range':
        $range = $data['range'];
        $range = str_replace(' - ', '/', $range);
        redirect(base_url().'hot_report_tax/range/'.$range);
        break;
  ...
  {
    $data['access'] = $this->access;
    $data['title'] = 'Laporan Pajak Tahun '.$year;
    $data['year'] = $year;
    //
    $data['annual'] = $this->m_hot_report_tax->annual($year);
    $data['client'] = $this->m_hot_client->get_all();
    $data['charge_type'] = $this->m_hot_charge_type->get_by_id('1');
    //
  ...
  {
    $title = 'Laporan Pajak Tahun '.$year;
    $client = $this->m_hot_client->get_all();
    //
    $annual = $this->m_hot_report_tax->annual($year);
  ...
  public function monthly($month)
  {
    ...
    $data['access'] = $this->access;
    $data['title'] = 'Laporan Pajak Bulan '.month_name_ind($num_month).' '.$raw[0];
    $data['month'] = $month;

    $data['monthly'] = $this->m_hot_report_tax->monthly($month);
    $data['client'] = $this->m_hot_client->get_all();
    $data['charge_type'] = $this->m_hot_charge_type->get_by_id('1');
    //
  ...
  public function monthly_print($month)
  {
    ...
    $num_month = $raw[1];
    //
    $title = "Laporan Pajak Bulan ".month_name_ind($num_month)."\n".$raw[0];
    $client = $this->m_hot_client->get_all();
    //
    $monthly = $this->m_hot_report_tax->monthly($month);
    $charge_type = $this->m_hot_charge_type->list_data_except_tax_hotel();
    //
  ...
  {
    $data['access'] = $this->access;
    $data['title'] = 'Laporan Pajak Mingguan ('.$date_start.' - '.$date_end.')';
    $data['date_start'] = $date_start;
    $data['date_end'] = $date_end;

    $data['weekly'] = $this->m_hot_report_tax->weekly(ind_to_date($date_start),ind_to_date($date_end));
    $data['client'] = $this->m_hot_client->get_all();
    $data['charge_type'] = $this->m_hot_charge_type->get_by_id('1');
    //
  ...
  {
    $title = "Laporan Pajak Mingguan \n (".$date_start." - ".$date_end.")";
    $client = $this->m_hot_client->get_all();
    //
    $weekly = $this->m_hot_report_tax->weekly(ind_to_date($date_start),ind_to_date($date_end));
    $charge_type = $this->m_hot_charge_type->list_data_except_tax_hotel();
    //
    ...
    //
    redirect(base_url().'hot_report_tax/weekly/'.$date_start.'/'.$date_end);
  }

  public function daily($date)
  {
    $data['access'] = $this->access;
    $data['title'] = 'Laporan Pajak Tanggal '.date_to_ind($date);
    $data['date'] = $date;
    //
    ...
    $data['daily'] = $this->m_hot_report_tax->daily($date);
    $data['client'] = $this->m_hot_client->get_all();
    $data['charge_type'] = $this->m_hot_charge_type->get_by_id('1');
    //
  ...
  {
    $title = "Laporan Pajak Tanggal \n".date_to_ind($date);
    $client = $this->m_hot_client->get_all();
    //
    $daily = $this->m_hot_report_tax->daily($date);
    $charge_type = $this->m_hot_charge_type->list_data_except_tax_hotel();
    //
  ...
  {
    $data['access'] = $this->access;
    $data['title'] = 'Laporan Pajak Tanggal '.$date_start.' - '.$date_end;
    $data['date_start'] = $date_start;
    $data['date_end'] = $date_end;

    $data['range'] = $this->m_hot_report_tax->range(ind_to_date($date_start),ind_to_date($date_end));
    $data['client'] = $this->m_hot_client->get_all();
    $data['charge_type'] = $this->m_hot_charge_type->get_by_id('1');
    //
  ...
  public function range_print($date_start, $date_end)
  {
    ...
    $client = $this->m_hot_client->get_all();
    //
    $range = $this->m_hot_report_tax->range(ind_to_date($date_start),ind_to_date($date_end));
    $charge_type = $this->m_hot_charge_type->list_data_except_tax_hotel();
    //
    ...
    //
    redirect(base_url().'hot_report_tax/range/'.$date_start.'/'.$date_end);
  }
  ...
  {
    $data = $_POST;
    $this->view('hot_report_tax/frame_pdf', $data);
  }
