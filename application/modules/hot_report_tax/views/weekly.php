  ...
  <div class="row">
    <div class="col-md-12">
      <form method="post" action="<?=base_url()?>hot_report_tax/frame_pdf">
        <input type="hidden" name="url" value="<?=base_url()?>hot_report_tax/weekly_pdf/<?=$date_start?>/<?=$date_end?>">
        <button class="btn btn-primary" type="submit"><i class="fa fa-print"></i> Download PDF</button>
        <a href="<?=base_url()?>hot_report_tax/weekly_print/<?=$date_start?>/<?=$date_end?>" class="btn btn-warning"><i class="fa fa-print"></i> Print Laporan</a>
      </form> 
      <br><br>
