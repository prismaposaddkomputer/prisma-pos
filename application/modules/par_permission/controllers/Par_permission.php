...
class Par_permission extends MY_Parking {

  var $access, $role_id, $type_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'par_permission'){
      $this->session->set_userdata(array('menu' => 'par_permission'));
      $this->session->unset_userdata('search_term');
    }
    $this->load->model('app_config/m_par_config');

    $this->role_id = $this->session->userdata('role_id');
    $this->module_controller = 'par_permission';
    $this->access = $this->m_par_config->get_permission($this->role_id, $this->module_controller);

    $this->load->model('par_role/m_par_role');
    $this->load->model('m_par_permission');
  }
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $config['base_url'] = base_url().'par_permission/index/';
      $config['per_page'] = 10;
      ...
      $this->view('par_permission/index',$data);
    } else {
  ...
  {
    $this->session->unset_userdata('search_term');
    redirect(base_url().'par_permission/index');
  }
  ...
  public function form($id = null)
  {
    ...
    if ($this->access->_update == 1) {
      ...
      $data['action'] = 'update';
      $this->load->model('m_par_permission');
      $data['permission'] = $this->m_par_permission->get_all($id);
      $this->view('par_permission/form', $data);
    } else {
      redirect(base_url().'app_error/error/403');
    }
  }
  ...
  public function insert()
  {
    ...
    $this->m_par_permission->insert($data);
  ...
  {
    $data['permission']= $this->m_par_permission->get_specific($id);
    $this->load->view('par_permission/update', $data);
  }

  public function update()
  {
    $data = $_POST;
    $this->load->model('par_permission/m_par_permission');

    $this->m_par_permission->empty_list($data['role_id']);
    ...
    foreach ($data2 as $key => $val) {
      ...
      $this->m_par_permission->insert($val);
    }
  ...
  public function delete($id)
  {
    if ($this->access->_delete == 1) {
      $this->m_par_permission->delete($id);
