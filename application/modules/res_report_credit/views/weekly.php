  ...
  <div class="row">
    <div class="col-md-12">
      <form method="post" action="<?=base_url()?>res_report_credit/frame_pdf">
        <input type="hidden" name="url" value="<?=base_url()?>res_report_credit/weekly_pdf/<?=$date_start?>/<?=$date_end?>">
        <button class="btn btn-primary" type="submit"><i class="fa fa-file-pdf-o"></i> Download PDF</button>
        <a href="<?=base_url()?>res_report_credit/weekly_print/<?=$date_start?>/<?=$date_end?>" class="btn btn-warning"><i class="fa fa-print"></i> Print Laporan</a>
      </form>
      <br>
            ...
            <?php $i=1;foreach ($weekly as $row): ?>
              <tr>
                ...
                <td class="text-center">
                  <a href="<?=base_url()?>res_report_credit/daily/<?=$row->tx_date?>" class="btn btn-xs btn-success"><i class="fa fa-list"></i> </a>
                </td>
