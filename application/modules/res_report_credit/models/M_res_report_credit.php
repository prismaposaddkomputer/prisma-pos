...
class M_res_report_credit extends CI_Model {
	...
	public function daily($date)
  {
		$credit = $this->db->query(
			...
			FROM res_billing a
			LEFT JOIN
				res_payment_type b ON a.payment_type_id = b.payment_type_id
			WHERE
				tx_date = '$date' AND
				tx_status = -1
			"
	...
	{
		$billing = $this->db
			->join('res_payment_type', 'res_billing.payment_type_id = res_payment_type.payment_type_id','left')
			->where('tx_id', $tx_id)
			->get('res_billing')->row();
