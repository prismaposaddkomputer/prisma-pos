...
class Kar_report_payment extends MY_Karaoke {

  var $access, $report_selling_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'kar_report_payment'){
      $this->session->set_userdata(array('menu' => 'kar_report_payment'));
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $this->view('kar_report_payment/index',$data);
    } else {
  ...
  public function action()
  {
    ...
    switch ($data['type']) {
      ...
      case 'annual':
        $year = $data['year'];
        redirect(base_url().'kar_report_payment/annual/'.$year);
        break;

      case 'monthly':
        $month = ind_to_month($data['month']);
        redirect(base_url().'kar_report_payment/monthly/'.$month);
        break;

      case 'weekly':
        $week = $data['week'];
        $week = str_replace(' - ', '/', $week);
        redirect(base_url().'kar_report_payment/weekly/'.$week);
        break;

      case 'daily':
        $date = ind_to_date($data['date']);
        redirect(base_url().'kar_report_payment/daily/'.$date);
        break;

      case 'range':
        $range = $data['range'];
        $range = str_replace(' - ', '/', $range);
        redirect(base_url().'kar_report_payment/range/'.$range);
        break;
  ...
  public function annual($year)
  {
    ...
    $data['year'] = $year;
    //
    $data['annual'] = $this->m_kar_report_payment->annual($year);
  ...
  public function annual_print($year)
  {
    ...
    //
    $annual = $this->m_kar_report_payment->annual($year);
    ...
    try {
      ...
      foreach ($annual as $row){
        ...
        //
        $dp_left = "DP";
        $dp_right = num_to_price($row->billing_down_payment);
        $printer -> text(print_justify($dp_left, $dp_right, 16, 13, 3));
        //
        $printer -> text('--------------------------------');
        $printer -> selectPrintMode(Escpos\Printer::MODE_EMPHASIZED);
        $total_left = "Total";
  ...
  public function monthly($month)
  {
    ...
    $data['monthly'] = $this->m_kar_report_payment->monthly($month);
    $data['charge_type'] = $this->m_kar_charge_type->list_data_except_tax_karaoke();
    //
  ...
  public function monthly_print($month)
  {
    ...
    $client = $this->m_kar_client->get_all();
    //
    $monthly = $this->m_kar_report_payment->monthly($month);
    $charge_type = $this->m_kar_charge_type->list_data_except_tax_karaoke();
    //
  ...
  public function weekly($date_start, $date_end)
  {
    ...
    $data['weekly'] = $this->m_kar_report_payment->weekly(ind_to_date($date_start),ind_to_date($date_end));
    $data['charge_type'] = $this->m_kar_charge_type->list_data_except_tax_karaoke();
    //
  ...
  public function weekly_print($date_start, $date_end)
  {
    ...
    $client = $this->m_kar_client->get_all();
    //
    $weekly = $this->m_kar_report_payment->weekly(ind_to_date($date_start),ind_to_date($date_end));
    $charge_type = $this->m_kar_charge_type->list_data_except_tax_karaoke();
    //
    ...
    //
    redirect(base_url().'kar_report_payment/weekly/'.$date_start.'/'.$date_end);
  }
  ...
  public function daily($date)
  {
    ...
    $data['daily'] = $this->m_kar_report_payment->daily($date);
    $data['charge_type'] = $this->m_kar_charge_type->list_data_except_tax_karaoke();
    //
  ...
  public function daily_print($date)
  {
    ...
    $client = $this->m_kar_client->get_all();
    //
    $daily = $this->m_kar_report_payment->daily($date);
    $charge_type = $this->m_kar_charge_type->list_data_except_tax_karaoke();
    //
  ...
  public function range($date_start, $date_end)
  {
    ...
    $data['range'] = $this->m_kar_report_payment->range(ind_to_date($date_start),ind_to_date($date_end));
    $data['charge_type'] = $this->m_kar_charge_type->list_data_except_tax_karaoke();
    //
  ...
  public function range_print($date_start, $date_end)
  {
    ...
    $client = $this->m_kar_client->get_all();
    //
    $range = $this->m_kar_report_payment->range(ind_to_date($date_start),ind_to_date($date_end));
    $charge_type = $this->m_kar_charge_type->list_data_except_tax_karaoke();
    //
    ...
    //
    redirect(base_url().'kar_report_payment/range/'.$date_start.'/'.$date_end);
  }
  ...
  {
    $data = $_POST;
    $this->view('kar_report_payment/frame_pdf', $data);
  }
