...
class Res_user extends MY_Restaurant {

  var $access, $role_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'res_user'){
      $this->session->set_userdata(array('menu' => 'res_user'));
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $config['base_url'] = base_url().'res_user/index/';
      $config['per_page'] = 10;

      $from = $this->uri->segment(3);

      if($this->session->userdata('search_term') == null){
        $num_rows = $this->m_res_user->num_rows();

        $config['total_rows'] = $num_rows;
        $this->pagination->initialize($config);

        $data['user'] = $this->m_res_user->get_list($config['per_page'],$from,$search_term = null);
      }else{
        $search_term = $this->session->userdata('search_term');
        $num_rows = $this->m_res_user->num_rows($search_term);
        $config['total_rows'] = $num_rows;
        $this->pagination->initialize($config);

        $data['user'] = $this->m_res_user->get_list($config['per_page'],$from,$search_term);
      }

      $this->view('res_user/index',$data);
    } else {
  ...
  {
    $this->session->unset_userdata('search_term');
    redirect(base_url().'res_user/index');
  }

  public function form($id = null)
  {
    $data['access'] = $this->access;
    $data['role_list'] = $this->m_res_role->get_all();
    if ($id == null) {
      if ($this->access->_create == 1) {
        $data['title'] = 'Tambah Pengguna';
        $data['action'] = 'insert';
        $data['user'] = null;
        $this->view('res_user/form', $data);
      } else {
        redirect(base_url().'app_error/error/403');
      }
    }else{
      if ($this->access->_update == 1) {
        ...
        $data['user'] = $this->m_res_user->get_by_id($id);
        $data['action'] = 'update';
        $this->view('res_user/form', $data);
      } else {
        redirect(base_url().'app_error/error/403');
  ...
  {
    $data['user']= $this->m_res_user->get_specific($id);
    $this->load->view('res_user/update', $data);
  }
  ...
  public function update()
  {
    ...
    $this->m_res_user->update($id,$data);
  ...
  public function update_password()
  {
    ...
    $current = $this->m_res_user->get_by_id($id);
    ...
    if(md5($data['user_password_old']) != $current->user_password){
      ...
      redirect(base_url().'res_user/form/'.$id);
