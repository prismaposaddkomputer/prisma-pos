...
class Kar_report_receptionist extends MY_Karaoke {

  var $access, $report_selling_user_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'kar_report_receptionist'){
      $this->session->set_userdata(array('menu' => 'kar_report_receptionist'));
    ...
    $this->module_controller = 'kar_report_receptionist';
    $this->access = $this->m_kar_config->get_permission($this->role_id, $this->module_controller);

    $this->load->model('m_kar_report_receptionist');
    ...
    $this->load->model('kar_reservation/m_kar_reservation');
  }
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $this->view('kar_report_receptionist/index',$data);
    } else {
  ...
  public function action()
  {
    ...
    switch ($data['type']) {
      ...
      case 'annual':
        $year = $data['year'];
        redirect(base_url().'kar_report_receptionist/annual/'.$year.'/'.$user_id);
        break;

      case 'monthly':
        $month = ind_to_month($data['month']);
        redirect(base_url().'kar_report_receptionist/monthly/'.$month.'/'.$user_id);
        break;

      case 'weekly':
        $week = $data['week'];
        $week = str_replace(' - ', '/', $week);
        redirect(base_url().'kar_report_receptionist/weekly/'.$week.'/'.$user_id);
        break;

      case 'daily':
        $date = ind_to_date($data['date']);
        redirect(base_url().'kar_report_receptionist/daily/'.$date.'/'.$user_id);
        break;

      case 'range':
        $range = $data['range'];
        $range = str_replace(' - ', '/', $range);
        redirect(base_url().'kar_report_receptionist/range/'.$range.'/'.$user_id);
        break;
  ...
  public function annual($year,$user_id)
  {
    ...
    $data['annual'] = $this->m_kar_report_receptionist->annual($year,$user_id);
    $data['year'] = $year;
  ...
  { 
    ...
    //
    $annual = $this->m_kar_report_receptionist->annual($year,$user_id);
    $charge_type = $this->m_kar_charge_type->list_data_except_tax_karaoke();
  ...
  public function monthly($month,$user_id)
  {
    ...
    $data['monthly'] = $this->m_kar_report_receptionist->monthly($month,$user_id);
    $data['charge_type'] = $this->m_kar_charge_type->list_data_except_tax_karaoke();
    //
  ...
  { 
    ...
    $client = $this->m_kar_client->get_all();
    //
    $monthly = $this->m_kar_report_receptionist->monthly($month,$user_id);
    $charge_type = $this->m_kar_charge_type->list_data_except_tax_karaoke();
  ...
  public function weekly($date_start, $date_end, $user_id)
  {
    ...
    $data['weekly'] = $this->m_kar_report_receptionist->weekly(ind_to_date($date_start),ind_to_date($date_end),$user_id);
    $data['charge_type'] = $this->m_kar_charge_type->list_data_except_tax_karaoke();
    //
  ...
  public function weekly_print($date_start, $date_end, $user_id)
  {
    ...
    $client = $this->m_kar_client->get_all();
    //
    $weekly = $this->m_kar_report_receptionist->weekly(ind_to_date($date_start),ind_to_date($date_end),$user_id);
    $charge_type = $this->m_kar_charge_type->list_data_except_tax_karaoke();
    ...
    //
    redirect(base_url().'kar_report_receptionist/weekly/'.$date_start.'/'.$date_end.'/'.$user_id);
  }
  ...
  public function daily($date, $user_id)
  {
    ...
    $data['daily'] = $this->m_kar_report_receptionist->daily($date, $user_id);
    $data['charge_type'] = $this->m_kar_charge_type->list_data_except_tax_karaoke();
    //
  ...
  public function daily_print($date, $user_id)
  {
    ...
    $client = $this->m_kar_client->get_all();
    //
    $daily = $this->m_kar_report_receptionist->daily($date, $user_id);
    $charge_type = $this->m_kar_charge_type->list_data_except_tax_karaoke();
  ...
  public function range($date_start, $date_end, $user_id)
  {
    ...
    $data['range'] = $this->m_kar_report_receptionist->range(ind_to_date($date_start),ind_to_date($date_end),$user_id);
    $data['charge_type'] = $this->m_kar_charge_type->list_data_except_tax_karaoke();
    //
  ...
  public function detail($tx_id)
  {
    ...
    $data['billing'] = $this->m_kar_reservation->get_billing($tx_id);
  ...
  public function range_print($date_start, $date_end, $user_id)
  {
    ...
    $client = $this->m_kar_client->get_all();
    //
    $range = $this->m_kar_report_receptionist->range(ind_to_date($date_start),ind_to_date($date_end),$user_id);
    $charge_type = $this->m_kar_charge_type->list_data_except_tax_karaoke();
    ...
    //
    redirect(base_url().'kar_report_receptionist/range/'.$date_start.'/'.$date_end.'/'.$user_id);
  }

  public function frame_pdf()
  {
    $data = $_POST;
    $this->view('kar_report_receptionist/frame_pdf', $data);
  }
