...
<div class="content-body">
  <div class="row">
    <form id="form" class="" action="<?=base_url()?>ret_item/<?=$action?>" method="post">
      <div class="col-md-6">
      ...
      <div class="row">
        <div class="col-md-12">
          <div class="form-group pull-right">
            <a class="btn btn-default" href="<?=base_url()?>ret_item/index"><i class="fa fa-close"></i> Batal</a>
          ...
          '<div class="col-md-7">'+
            ...
            '<select class="form-control keyboard select2" name="item_detail_id[]">'+
              <?php foreach ($item_list as $row): ?>
                '<option value="<?=$row->item_id?>"><?=$row->item_name?> (<?=num_to_idr($row->item_price_before_tax)?>)</option>'+
              <?php endforeach; ?>
            '</select>'+
          '</div>'+
          '<div class="col-md-3">'+
            '<label>Harga Paket</label>'+
            '<input class="form-control keyboard" type="number" name="item_detail_price[]" oninput="cacl_price()">'+
          '</div>'+
          '<div class="col-md-1">'+
            '<label>&nbsp;</label>'+
            '<button class="btn btn-danger" type="button" onclick="remove_package('+package_row+')"><i class="fa fa-minus"></i></button>'+
