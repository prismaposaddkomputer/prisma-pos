...
class M_ret_report_credit extends CI_Model {
	...
	public function daily($date)
  {
		$credit = $this->db->query(
			...
			FROM ret_billing a
			LEFT JOIN
				ret_payment_type b ON a.payment_type_id = b.payment_type_id
			WHERE 
				tx_date = '$date' AND
				tx_status = -1
			"
	...
	{
		$billing = $this->db
			->join('ret_payment_type', 'ret_billing.payment_type_id = ret_payment_type.payment_type_id','left')
			->where('tx_id', $tx_id)
			->get('ret_billing')->row();
