...
class Ret_report_credit extends MY_Retail {

  var $access, $report_credit_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'ret_report_credit'){
      $this->session->set_userdata(array('menu' => 'ret_report_credit'));
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $this->view('ret_report_credit/index',$data);
    } else {
  ...
  public function action()
  {
    ...
    switch ($data['type']) {
      ...
      case 'annual':
        $year = $data['year'];
        redirect(base_url().'ret_report_credit/annual/'.$year);
        break;

      case 'monthly':
        $month = ind_to_month($data['month']);
        redirect(base_url().'ret_report_credit/monthly/'.$month);
        break;

      case 'weekly':
        $week = $data['week'];
        $week = str_replace(' - ', '/', $week);
        redirect(base_url().'ret_report_credit/weekly/'.$week);
        break;

      case 'daily':
        $date = ind_to_date($data['date']);
        redirect(base_url().'ret_report_credit/daily/'.$date);
        break;

      case 'range':
        $range = $data['range'];
        $range = str_replace(' - ', '/', $range);
        redirect(base_url().'ret_report_credit/range/'.$range);
        break;
  ...
  public function annual($year)
  {
    ...
    $data['annual'] = $this->m_ret_report_credit->annual($year);
  ...
  public function monthly($month)
  {
    ...
    $data['monthly'] = $this->m_ret_report_credit->monthly($month);
  ...
  public function weekly($date_start, $date_end)
  {
    ...
    $data['weekly'] = $this->m_ret_report_credit->weekly(ind_to_date($date_start),ind_to_date($date_end));
  ...
  public function daily($date)
  {
    ...
    $data['daily'] = $this->m_ret_report_credit->daily($date);
  ...
  public function range($date_start, $date_end)
  {
    ...
    $data['range'] = $this->m_ret_report_credit->range(ind_to_date($date_start),ind_to_date($date_end));
  ...
  public function detail($tx_id)
  {
    ...
    $data['billing'] = $this->m_ret_report_credit->detail($tx_id);
