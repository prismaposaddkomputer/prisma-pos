...
class Kar_role extends MY_Karaoke {

  var $access, $role_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'kar_role'){
      $this->session->set_userdata(array('menu' => 'kar_role'));
      $this->session->unset_userdata('search_term');
    }
    $this->load->model('app_config/m_kar_config');

    $this->role_id = $this->session->userdata('role_id');
    $this->module_controller = 'kar_role';
    $this->access = $this->m_kar_config->get_permission($this->role_id, $this->module_controller);

    $this->load->model('m_kar_role');
  }
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $config['base_url'] = base_url().'kar_role/index/';
      $config['per_page'] = 10;
  ...
  {
    $this->session->unset_userdata('search_term');
    redirect(base_url().'kar_role/index');
  }
  ...
  public function form($id = null)
  {
    ...
    if ($id == null) {
      ...
      if ($this->access->_update == 1) {
        ...
        $data['role'] = $this->m_kar_role->get_by_id($id);
        $data['action'] = 'update';
        $this->view('kar_role/form', $data);
      } else {
        redirect(base_url().'app_error/error/403');
  ...
  {
    $data['role']= $this->m_kar_role->get_specific($id);
    $this->load->view('kar_role/update', $data);
  }
  ...
  public function update()
  {
    ...
    $this->m_kar_role->update($id,$data);
