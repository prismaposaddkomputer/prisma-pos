...
<div class="content-body">
  <div class="row">
    <div class="col-md-4">
      <a class="btn btn-info" href="<?=base_url()?>ret_po/form"><i class="fa fa-plus"></i> Tambah Purchase Order</a>
    </div>
    <div class="col-md-8 col-lg-7 pull-right">
      <form class="" action="<?=base_url()?>ret_po/index" method="post">
        <div class="form-group">
          <div class="input-group">
            ...
            <span class="input-group-btn">
              ...
              <a class="btn btn-default" href="<?=base_url()?>ret_po/reset_search"><i class="fa fa-refresh"></i></a>
            </span>
          </div>
        </div>
    </div>
        ...
        <table class="table table-striped table-bordered table-condensed">
          <thead>
            <tr>
              ...
              <th class="text-center" width="100">No Kwitansi</th>
              <th class="text-center" width="100">Tanggal</th>
              <th class="text-center" width="60">Status</th>
              <th class="text-center" width="150">No PO</th>
              <th class="text-center" width="250">Suplier</th>
              <th class="text-center">Catatan</th>
            </tr>
          </thead>
          <tbody>
            <?php if ($po != null): ?>
              <?php $i=1;foreach ($po as $row): ?>
                <tr>
                  <td class="text-center"><?=$this->uri->segment('3')+$i++?></td>
                  <td class="text-center">
                    <?php if ($row->tx_id != '' ): ?>
                      <?php if ($row->tx_status == 0): ?>
                        <a class="btn btn-xs btn-primary" href="<?=base_url()?>ret_po/form/<?=$row->tx_id?>"><i class="fa fa-bookmark"></i></a>
                      <?php endif; ?>
                      <a class="btn btn-xs btn-success" href="<?=base_url()?>ret_po/detail/<?=$row->tx_id?>/<?=$row->tx_type?>"><i class="fa fa-list"></i></a>
                    <?php endif; ?>
                  </td>
                  ...
                  <td class="text-center">
                    <?php if ($row->tx_status == 0): ?>
                      <b class="cl-warning">Proses</b>
                    <?php elseif($row->tx_status == 1): ?>
                      <b class="cl-success">Selesai</b>
                    <?php elseif($row->tx_status == -1): ?>
                      <b class="cl-danger">Batal</b>
                    <?php endif; ?>
                  </td>
                  <td><?=$row->tx_po_no?></td>
...
<script type="text/javascript">
  function del(id) {
    ...
    $("#btn_delete_action").click(function () {
      window.location = "<?=base_url()?>ret_po/delete/"+id;
    })
  }
</script>
