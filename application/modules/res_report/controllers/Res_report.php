...
class Res_report extends MY_Restaurant {

  var $access, $report_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'res_report'){
      $this->session->set_userdata(array('menu' => 'res_report'));
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $this->view('res_report/index',$data);
    } else {
  ...
  public function report_stock()
  {
    ...
    if($this->session->userdata('search_stock') == null){
      $data['stock_recap'] = $this->m_res_report->report_stock($search_stock = null);
    }else{
      $search_stock = $this->session->userdata('search_stock');

      $data['stock_recap'] = $this->m_res_report->report_stock($search_stock);
    }

    $this->view('res_report/report_stock',$data);
  }

  public function reset_stock()
  {
    $this->session->unset_userdata('search_stock');
    redirect(base_url().'res_report/report_stock');
  }
  ...
  public function report_selling($type = null)
  {
    ...
    $this->view('res_report/report_selling',$data);
  }
  ...
  public function report_selling_action()
  {
    ...
    switch ($data['type']) {
      ...
      case 'annual':
        $year = $data['year'];
        redirect(base_url().'res_report/report_selling_annual/'.$year);
        break;

      case 'monthly':
        $month = ind_to_month($data['month']);
        redirect(base_url().'res_report/report_selling_monthly/'.$month);
        break;

      case 'weekly':
        if($data['week'] == ''){
          redirect(base_url().'res_report/report_selling/weekly');
        };
        $week = $data['week'];
        $week = str_replace(' - ', '/', $week);
        redirect(base_url().'res_report/report_selling_weekly/'.$week);
        break;

      case 'daily':
        $date = ind_to_date($data['date']);
        redirect(base_url().'res_report/report_selling_daily/'.$date);
        break;

      case 'range':
        $range = $data['range'];
        $range = str_replace(' - ', '/', $range);
        redirect(base_url().'res_report/report_selling_range/'.$range);
        break;
  ...
  public function report_selling_annual($year)
  {
    ...
    $data['annual'] = $this->m_res_report->report_selling_annual($year);
  ...
  public function report_selling_monthly($month)
  {
    ...
    $data['monthly'] = $this->m_res_report->report_selling_monthly($month);
  ...
  public function report_selling_weekly($date_start, $date_end)
  {
    ...
    $data['weekly'] = $this->m_res_report->report_selling_weekly(ind_to_date($date_start),ind_to_date($date_end));
  ...
  public function report_selling_daily($date)
  {
    ...
    $data['daily'] = $this->m_res_report->report_selling_daily($date);
  ...
  public function report_selling_range($date_start, $date_end)
  {
    ...
    $data['range'] = $this->m_res_report->report_selling_range(ind_to_date($date_start),ind_to_date($date_end));
  ...
  public function report_selling_detail($tx_id)
  {
    ...
    $data['billing'] = $this->m_res_report->report_selling_detail($tx_id);
  ...
  {
    $this->session->unset_userdata('search_selling');
    redirect(base_url().'res_report/report_selling');
  }
  ...
  public function report_selling_item()
  {
    ...
    if($this->session->userdata('search_selling_item') == null){
      $data['selling_item'] = $this->m_res_report->report_selling_item($search_selling_item = null);
    }else{
      $search_selling_item = $this->session->userdata('search_selling_item');

      $data['selling_item'] = $this->m_res_report->report_selling_item($search_selling_item);
    }

    $this->view('res_report/report_selling_item',$data);
  }

  public function reset_selling_item()
  {
    $this->session->unset_userdata('search_selling_item');
    redirect(base_url().'res_report/report_selling_item');
  }
  ...
  public function report_profit_daily()
  {
    ...
    if($this->session->userdata('search_profit_daily') == null){
      $data['profit_daily'] = $this->m_res_report->report_profit_daily($search_profit_daily = null);
    }else{
      $search_profit_daily = $this->session->userdata('search_profit_daily');

      $data['profit_daily'] = $this->m_res_report->report_profit_daily($search_profit_daily);
    }

    $this->view('res_report/report_profit_daily',$data);
  }

  public function reset_profit_daily()
  {
    $this->session->unset_userdata('search_profit_daily');
    redirect(base_url().'res_report/report_profit_daily');
  }
  ...
  public function report_profit_item()
  {
    ...
    if($this->session->userdata('search_profit_item') == null){
      $data['profit_item'] = $this->m_res_report->report_profit_item($search_profit_item = null);
    }else{
      $search_profit_item = $this->session->userdata('search_profit_item');

      $data['profit_item'] = $this->m_res_report->report_profit_item($search_profit_item);
    }

    $this->view('res_report/report_profit_item',$data);
  }

  public function reset_profit_item()
  {
    $this->session->unset_userdata('search_profit_item');
    redirect(base_url().'res_report/report_profit_item');
  }
