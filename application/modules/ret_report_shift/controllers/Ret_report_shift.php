...
class Ret_report_shift extends MY_Retail {

  var $access, $report_shift_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'ret_report_shift'){
      $this->session->set_userdata(array('menu' => 'ret_report_shift'));
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $this->view('ret_report_shift/index',$data);
    } else {
  ...
  public function action()
  {
    ...
    switch ($data['type']) {
      ...
      case 'annual':
        $year = $data['year'];
        redirect(base_url().'ret_report_shift/annual/'.$year);
        break;

      case 'monthly':
        $month = ind_to_month($data['month']);
        redirect(base_url().'ret_report_shift/monthly/'.$month);
        break;

      case 'weekly':
        $week = $data['week'];
        $week = str_replace(' - ', '/', $week);
        redirect(base_url().'ret_report_shift/weekly/'.$week);
        break;

      case 'daily':
        $date = ind_to_date($data['date']);
        redirect(base_url().'ret_report_shift/daily/'.$date);
        break;

      case 'range':
        $range = $data['range'];
        $range = str_replace(' - ', '/', $range);
        redirect(base_url().'ret_report_shift/range/'.$range);
        break;
  ...
  public function annual($year)
  {
    ...
    $data['annual'] = $this->m_ret_report_shift->annual($year);
  ...
  public function monthly($month)
  {
    ...
    $data['monthly'] = $this->m_ret_report_shift->monthly($month);
  ...
  public function weekly($date_start, $date_end)
  {
    ...
    $data['weekly'] = $this->m_ret_report_shift->weekly(ind_to_date($date_start),ind_to_date($date_end));
  ...
  public function daily($date)
  {
    ...
    $data['daily'] = $this->m_ret_report_shift->daily($date);
  ...
  public function range($date_start, $date_end)
  {
    ...
    $data['range'] = $this->m_ret_report_shift->range(ind_to_date($date_start),ind_to_date($date_end));
  ...
  public function detail($tx_id)
  {
    ...
    $data['billing'] = $this->m_ret_report_shift->detail($tx_id);
