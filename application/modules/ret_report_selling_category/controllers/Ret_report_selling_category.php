...
class Ret_report_selling_category extends MY_Retail {

  var $access, $report_selling_category_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'ret_report_selling_category'){
      $this->session->set_userdata(array('menu' => 'ret_report_selling_category'));
    ...
    $this->module_controller = 'ret_report_selling_category';
    $this->access = $this->m_ret_config->get_permission($this->role_id, $this->module_controller);

    $this->load->model('m_ret_report_selling_category');
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $this->view('ret_report_selling_category/index',$data);
    } else {
  ...
  public function action()
  {
    ...
    switch ($data['type']) {
      ...
      case 'annual':
        $year = $data['year'];
        redirect(base_url().'ret_report_selling_category/annual/'.$year);
        break;

      case 'monthly':
        $month = ind_to_month($data['month']);
        redirect(base_url().'ret_report_selling_category/monthly/'.$month);
        break;

      case 'weekly':
        $week = $data['week'];
        $week = str_replace(' - ', '/', $week);
        redirect(base_url().'ret_report_selling_category/weekly/'.$week);
        break;

      case 'daily':
        $date = ind_to_date($data['date']);
        redirect(base_url().'ret_report_selling_category/daily/'.$date);
        break;

      case 'range':
        $range = $data['range'];
        $range = str_replace(' - ', '/', $range);
        redirect(base_url().'ret_report_selling_category/range/'.$range);
        break;
  ...
  public function annual($year)
  {
    ...
    $data['annual'] = $this->m_ret_report_selling_category->annual($year);
  ...
  public function monthly($month)
  {
    ...
    $data['monthly'] = $this->m_ret_report_selling_category->monthly($month);
  ...
  public function weekly($date_start, $date_end)
  {
    ...
    $data['weekly'] = $this->m_ret_report_selling_category->weekly(ind_to_date($date_start),ind_to_date($date_end));
  ...
  public function daily($date)
  {
    ...
    $data['daily'] = $this->m_ret_report_selling_category->daily($date);
  ...
  public function range($date_start, $date_end)
  {
    ...
    $data['range'] = $this->m_ret_report_selling_category->range(ind_to_date($date_start),ind_to_date($date_end));
