  ...
  <div class="row">
    <div class="col-md-12">
      <form method="post" action="<?=base_url()?>kar_report_credit/frame_pdf">
        <input type="hidden" name="url" value="<?=base_url()?>kar_report_credit/annual_pdf/<?=$year?>">
        <button class="btn btn-primary" type="submit"><i class="fa fa-print"></i> Download PDF</button>
        <a href="<?=base_url()?>kar_report_credit/annual_print/<?=$year?>" class="btn btn-warning"><i class="fa fa-print"></i> Print Laporan</a>
      </form>
      <br><br>
            ...
            <?php $i=1;foreach ($annual as $row): ?>
              <tr>
                ...
                <td class="text-center">
                  <a href="<?=base_url()?>kar_report_credit/monthly/<?=$row->tx_year?>-<?=str_pad($row->tx_month, 2, '0', STR_PAD_LEFT)?>" class="btn btn-xs btn-success"><i class="fa fa-list"></i> </a>
                </td>
