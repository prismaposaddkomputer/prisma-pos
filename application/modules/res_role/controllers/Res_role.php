...
class Res_role extends MY_Restaurant {

  var $access, $role_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'res_role'){
      $this->session->set_userdata(array('menu' => 'res_role'));
      $this->session->unset_userdata('search_term');
    }
    $this->load->model('app_config/m_res_config');

    $this->role_id = $this->session->userdata('role_id');
    $this->module_controller = 'res_role';
    $this->access = $this->m_res_config->get_permission($this->role_id, $this->module_controller);

    $this->load->model('m_res_role');
  }
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $config['base_url'] = base_url().'res_role/index/';
      $config['per_page'] = 10;
  ...
  {
    $this->session->unset_userdata('search_term');
    redirect(base_url().'res_role/index');
  }
  ...
  public function form($id = null)
  {
    ...
    if ($id == null) {
      ...
      if ($this->access->_update == 1) {
        ...
        $data['role'] = $this->m_res_role->get_by_id($id);
        $data['action'] = 'update';
        $this->view('res_role/form', $data);
      } else {
        redirect(base_url().'app_error/error/403');
  ...
  {
    $data['role']= $this->m_res_role->get_specific($id);
    $this->load->view('res_role/update', $data);
  }
  ...
  public function update()
  {
    ...
    $this->m_res_role->update($id,$data);
