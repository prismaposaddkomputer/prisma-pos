...
class Hot_report_reservation extends MY_Hotel {

  var $access, $report_selling_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'hot_report_reservation'){
      $this->session->set_userdata(array('menu' => 'hot_report_reservation'));
    ...
    $this->module_controller = 'hot_report_reservation';
    $this->access = $this->m_hot_config->get_permission($this->role_id, $this->module_controller);

    $this->load->model('m_hot_report_reservation');
  ...
  public function action()
  {
    ...
    switch ($data['type']) {
      ...
      case 'annual':
        $year = $data['year'];
        redirect(base_url().'hot_report_reservation/annual/'.$year);
        break;

      case 'monthly':
        $month = ind_to_month($data['month']);
        redirect(base_url().'hot_report_reservation/monthly/'.$month);
        break;

      case 'weekly':
        $week = $data['week'];
        $week = str_replace(' - ', '/', $week);
        redirect(base_url().'hot_report_reservation/weekly/'.$week);
        break;

      case 'daily':
        $date = ind_to_date($data['date']);
        redirect(base_url().'hot_report_reservation/daily/'.$date);
        break;

      case 'range':
        $range = $data['range'];
        $range = str_replace(' - ', '/', $range);
        redirect(base_url().'hot_report_reservation/range/'.$range);
        break;
  ...
  public function weekly_print($date_start, $date_end)
  {
    ...
    //
    redirect(base_url().'hot_report_reservation/weekly/'.$date_start.'/'.$date_end);
  }
  ...
  public function range_print($date_start, $date_end)
  {
    ...
    //
    redirect(base_url().'hot_report_reservation/range/'.$date_start.'/'.$date_end);
  }
