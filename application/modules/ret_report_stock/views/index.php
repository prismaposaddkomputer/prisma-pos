    ...
    <div class="col-md-2">
      <div class="form-group">
        ...
        <select class="form-control select2" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
          <option value="<?=base_url()?>ret_report_stock/index/daily" <?php if($type == 'daily'){echo 'selected';}?>>Harian</option>
          <option value="<?=base_url()?>ret_report_stock/index/weekly" <?php if($type == 'weekly'){echo 'selected';}?>>Mingguan</option>
          <option value="<?=base_url()?>ret_report_stock/index/monthly" <?php if($type == 'monthly'){echo 'selected';}?>>Bulanan</option>
          <option value="<?=base_url()?>ret_report_stock/index/annual" <?php if($type == 'annual'){echo 'selected';}?>>Tahunan</option>
          <option value="<?=base_url()?>ret_report_stock/index/range" <?php if($type == 'range'){echo 'selected';}?>>Rentang Waktu</option>
        </select>
      </div>
    </div>
    <?php if ($type == 'daily'): ?>
      <div class="col-md-3">
        <form class="" action="<?=base_url()?>ret_report_stock/action" method="post">
