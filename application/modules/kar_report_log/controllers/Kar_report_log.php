...
class Kar_report_log extends MY_Karaoke {

  var $access, $report_log_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'kar_report_log'){
      $this->session->set_userdata(array('menu' => 'kar_report_log'));
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $this->view('kar_report_log/index',$data);
    } else {
  ...
  public function action()
  {
    ...
    switch ($data['type']) {
      ...
      case 'annual':
        $year = $data['year'];
        redirect(base_url().'kar_report_log/annual/'.$year);
        break;

      case 'monthly':
        $month = ind_to_month($data['month']);
        redirect(base_url().'kar_report_log/monthly/'.$month);
        break;

      case 'weekly':
        $week = $data['week'];
        $week = str_replace(' - ', '/', $week);
        redirect(base_url().'kar_report_log/weekly/'.$week);
        break;

      case 'daily':
        $date = ind_to_date($data['date']);
        redirect(base_url().'kar_report_log/daily/'.$date);
        break;

      case 'range':
        $range = $data['range'];
        $range = str_replace(' - ', '/', $range);
        redirect(base_url().'kar_report_log/range/'.$range);
        break;
  ...
  public function annual($year)
  {
    ...
    $data['annual'] = $this->m_kar_report_log->annual($year);
  ...
  public function monthly($month)
  {
    ...
    $data['monthly'] = $this->m_kar_report_log->monthly($month);
  ...
  public function weekly($date_start, $date_end)
  {
    ...
    $data['weekly'] = $this->m_kar_report_log->weekly(ind_to_date($date_start),ind_to_date($date_end));
  ...
  public function daily($date)
  {
    ...
    $data['daily'] = $this->m_kar_report_log->daily($date);
  ...
  public function range($date_start, $date_end)
  {
    ...
    $data['range'] = $this->m_kar_report_log->range(ind_to_date($date_start),ind_to_date($date_end));
  ...
  public function detail($tx_id)
  {
    ...
    $data['billing'] = $this->m_kar_report_log->detail($tx_id);
