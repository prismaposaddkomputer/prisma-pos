...
class Ret_permission extends MY_Retail {

  var $access, $role_id, $type_id;

  function __construct(){
    parent::__construct();
    if($this->session->userdata('menu') != 'ret_permission'){
      $this->session->set_userdata(array('menu' => 'ret_permission'));
      $this->session->unset_userdata('search_term');
    }
    $this->load->model('app_config/m_ret_config');

    $this->role_id = $this->session->userdata('role_id');
    $this->module_controller = 'ret_permission';
    $this->access = $this->m_ret_config->get_permission($this->role_id, $this->module_controller);

    $this->load->model('ret_role/m_ret_role');
    $this->load->model('m_ret_permission');
  }
	...
	public function index()
  {
    if ($this->access->_read == 1) {
      ...
      $config['base_url'] = base_url().'ret_permission/index/';
      $config['per_page'] = 10;

      $from = $this->uri->segment(3);

      if($this->session->userdata('search_term') == null){
        $num_rows = $this->m_ret_role->num_rows();

        $config['total_rows'] = $num_rows;
        $this->pagination->initialize($config);

        $data['role'] = $this->m_ret_role->get_list($config['per_page'],$from,$search_term = null);
      }else{
        $search_term = $this->session->userdata('search_term');
        $num_rows = $this->m_ret_role->num_rows($search_term);
        $config['total_rows'] = $num_rows;
        $this->pagination->initialize($config);

        $data['role'] = $this->m_ret_role->get_list($config['per_page'],$from,$search_term);
      }

      $this->view('ret_permission/index',$data);
    } else {
  ...
  {
    $this->session->unset_userdata('search_term');
    redirect(base_url().'ret_permission/index');
  }
  ...
  public function form($id = null)
  {
    ...
    if ($this->access->_update == 1) {
      ...
      $data['action'] = 'update';
      $this->load->model('m_ret_permission');
      $data['permission'] = $this->m_ret_permission->get_all($id);
      $this->view('ret_permission/form', $data);
    } else {
      redirect(base_url().'app_error/error/403');
    }
  }
  ...
  public function insert()
  {
    ...
    $this->m_ret_permission->insert($data);
  ...
  {
    $data['permission']= $this->m_ret_permission->get_specific($id);
    $this->load->view('ret_permission/update', $data);
  }

  public function update()
  {
    $data = $_POST;
    $this->load->model('ret_permission/m_ret_permission');

    $this->m_ret_permission->empty_list($data['role_id']);
    ...
    foreach ($data2 as $key => $val) {
      ...
      $this->m_ret_permission->insert($val);
    }
  ...
  public function delete($id)
  {
    if ($this->access->_delete == 1) {
      $this->m_ret_permission->delete($id);
